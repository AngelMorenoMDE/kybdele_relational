 ...
 * TODO JLS
 */
public class AttributeIsNNTool extends TargetingTool{
	
	@Override
	protected Cursor getDefaultCursor() {
		Cursor c = super.getDefaultCursor();
//		c = new Cursor(Display.getDefault(), SWT.CURSOR_HAND);
		Image cursorPK = new Image(Display.getDefault(),AttributeIsPKTool.class.getResourceAsStream("/pointers/pointerNN.png"));
	...
	protected String getCommandName() {
		...
		return "AttributeIsNNCreateCommand";
	}
	
	public AttributeIsNNTool() {
	...
	public void mouseDown(MouseEvent me, EditPartViewer viewer) {
		...
		if (p.getClass() == AttributeEditPart.class || p.getParent().getClass() == AttributeEditPart.class){
			...
			boolean flagBoolean = !((AttributeImpl)model).isIsNN();
			
			Command setVariable = SetCommand.create(ted, model,Gmf_relational_modelPackage.Literals.ATTRIBUTE__IS_NN, flagBoolean);
