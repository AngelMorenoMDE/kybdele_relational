...
public class AttributeNameEditPart extends CompartmentEditPart implements
	 ...
	 * set font bold
	 */
	protected void setFontBold(boolean isBold) {
		if (isBold) {
			FontData fontData = new FontData("verdana", 12, SWT.BOLD);
			setFont(fontData);
		} else {
			FontData fontData = new FontData("verdana", 12, SWT.NORMAL);
			setFont(fontData);
		}
	}
	 ...
	 * set font italic
	 */
	protected void setFontItalic(boolean isItalic) {
		FontStyle style = (FontStyle) getFontStyleOwnerView().getStyle(
				NotationPackage.eINSTANCE.getFontStyle());
		if (isItalic) {
			FontData fontData = new FontData("verdana", 12, SWT.ITALIC);
			setFont(fontData);
		} else {
			FontData fontData = new FontData("verdana", 12, SWT.NORMAL);
			setFont(fontData);
