...
public class WrappingLabelEditPart extends CompartmentEditPart implements
	...
	protected void createDefaultEditPolicies() {
		...
		installEditPolicy(
				...
				new gmf_relational_model.gmf_relational_model.diagram.edit.policies.RelationalModelTextSelectionEditPolicy());
	...
	public IParser getParser() {
		if (parser == null) {
			parser = gmf_relational_model.gmf_relational_model.diagram.providers.RelationalModelParserProvider
					.getParser(
	...
	protected DirectEditManager getManager() {
		if (manager == null) {
			setManager(new TextDirectEditManager(
					this,
					null,
					gmf_relational_model.gmf_relational_model.diagram.edit.parts.RelationalModelEditPartFactory
	 ...
	 * TODO JLS
	 * 
	 * En este metodo se setea el valor de los flags isNN e isUN
	 * para tratar el literal que se muestra en AttributeFlagFigure,
	 * decidiendo entre Unique y *.
	 * 
	 * Se verifica tambien el literal cuando se crea PK no solo cuando
	 * se cambian los flag
	 * 
	 * VERIFICADO
	 */
	protected void handleNotificationEvent(Notification event) {
		...
		if (Notification.SET == event.getEventType()) {
			if (feature.getClass() == EAttributeImpl.class) {
				EAttributeImpl attAux = (EAttributeImpl) feature;
				if (event.getNotifier() instanceof AttributeImpl) {
					AttributeImpl attAuxImpl = (AttributeImpl) event
							.getNotifier();
					if ("isNN".equals(attAux.getName())) {
						if (event.getNewBooleanValue()) {
							setLabelText("");
						} else {
							//is Nullable
							if (attAuxImpl.isIsUN()) {
								//is NN and Unique
								setLabelText("");
							} else {
								//is NN and not Unique
								setNormalPosition();
								setLabelText("*");
							}
						}
					} else if ("isUN".equals(attAux.getName())) {
						if (event.getNewBooleanValue()) {
							//is Unique ,  top
							setLabelText("");
						} else {
							//is not unique
							if (attAuxImpl.isIsNN()) {
								//is NN and not Unique
								setLabelText("");
							} else {
								//is not NN and not Unique
								setLabelText("*");
							}
						}
					} else if ("isPK".equals(attAux.getName())) {
						setLabelText("");
					}
				}
			}
			if (feature.getClass() == EReferenceImpl.class) {
				EReferenceImpl refAux = (EReferenceImpl) feature;
				if ("pkReference".equals(refAux.getName())) {
	 ...
	 * TODO JLS
	 * 
	 * con este metodo disminuimos el tamaño de la letra que esta alineada arriba
	 * dando la impresion de que es un super indice para el flag UNIQUE
	 */
	public void setUpPosition() {
		Font font = ((WrappingLabel) this.getFigure()).getFont();
		FontData fd = ((FontData) font.getFontData()[0]);
		fd.setHeight(7);

		Font newFont = getResourceManager().createFont(
				FontDescriptor.createFrom(fd));

		((WrappingLabel) this.getFigure()).setFont(newFont);
	}
	 ...
	 * TODO JLS
	 * 
	 * con este metodo volvemos al tamaño normal de la letra para anular
	 * el efecto del metodo anterior , setUpPosition
	 */
