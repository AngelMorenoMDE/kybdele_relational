package gmf_relational_model.gmf_relational_model.diagram.edit.parts.custom;
...
public class DefaultSizeNodeFigureWithFixedAnchors extends
	...
	public ConnectionAnchor getSourceConnectionAnchorAt(Point p) {
		return findNearestAnchorFrom(p,0);
	}

	@Override
	public ConnectionAnchor getTargetConnectionAnchorAt(Point p) {
		return findNearestAnchorFrom(p,1);
	}
 ...
 * @param flag = 0 -> source ; flag = 1 -> target
 * @return
 */
	private ConnectionAnchor findNearestAnchorFrom(Point point,int flag) {
		...
		if (point == null || anchors.size() == 1) {
			...
			while (terminals.hasNext()) {
				String terminal = terminals.next();
//				if ("SOUTH".equals(terminal) && flag == 0){
				if ("SOUTH".equals(terminal)){
					FixedConnectionAnchor anchor = anchors.get(terminal);
...
//				}else if ("NORTH".equals(terminal) && flag == 1){
				}else if ("NORTH".equals(terminal)){
					FixedConnectionAnchor anchor = anchors.get(terminal);
